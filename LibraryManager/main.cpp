...
void sqlConnection()
{
    ...
    // 创建 userInfo 表
    query.exec(QString::fromUtf8("CREATE TABLE IF NOT EXISTS userInfo(账号 VARCHAR(20) PRIMARY KEY, 密码 VARCHAR(20), 已借书 INT, 共借书 INT, 逾期记录 VARCHAR(20))"));
    query.exec(QString::fromUtf8("INSERT INTO userInfo VALUES('admin', '%1', 0, 0, '无')").arg(password));
